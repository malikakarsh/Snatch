...
function getStatusLabel(engagement: Engagement): string {
  ...
  switch (engagement.status) {
    ...
    case 'PHASE_2_LIVE':
      return engagement.auctionFormat === 'OPEN' ? 'Live Auction' : 'Phase 2 Live';
    case 'CLOSED':
      return 'Closed';
    case 'CANCELLED':
      return 'Cancelled';
...
export default function AuctionCard({
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        client.subscribe(`/topic/engagements/${engagement.id}/status`, (msg) => {
          const update = JSON.parse(msg.body) as { status: string; reason?: string };
          setEngagement(prev => ({
            ...prev,
    ...
    return (
      <button
        ...
        className={`text-2xl leading-none transition-colors disabled:opacity-50 ${
          favorited ? 'text-amber-400 hover:text-amber-300' : 'text-zinc-500 hover:text-zinc-300'
        }`}
      >
        {favorited ? '★' : '☆'}
      </button>
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              {engagement.status === 'CANCELLED' && engagement.cancelReason === 'NO_PARTICIPANTS' && (
              ...
              {engagement.status === 'CLOSED' && engagement.cancelReason === 'STOPPED_BY_AUCTIONEER' && (
                <div className="px-2 py-0.5 rounded border text-[9px] font-bold tracking-widest uppercase bg-amber-500/10 text-amber-400 border-amber-500/20 shrink-0">
                  Stopped Early
                </div>
              )}
              {engagement.status === 'CLOSED' && engagement.winnerId && !isOpenFormat && (
          ...
          <div className="flex items-center gap-4 shrink-0">
            {!isOpenFormat && isPhase1 && engagement.maxStartingRate != null && (
            ...
            {!isOpenFormat && isPhase2 && engagement.currentLiveRate != null && userRole !== 'BIDDER' && (
              <div className="text-right">
            ...
            {!isOpenFormat && engagement.status === 'CLOSED' && engagement.currentLiveRate != null && userRole !== 'BIDDER' && (
              <div className="text-right">
            ...
            {!isOpenFormat && engagement.status === 'CLOSED' && engagement.currentLiveRate != null && userRole === 'BIDDER' && engagement.winnerId === user?.email && (
              <div className="text-right">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 pt-3 border-t border-white/5">
          <span className="text-[10px] text-zinc-500">
            {engagement.auctionType === 'DESCENDING'
              ? '🔻 Dutch'
              : isOpenFormat ? '🎪 Open English' : '🔺 English'}
          </span>
          ...
          {isOpenFormat && engagement.openStartTime && engagement.status === 'PENDING' && (
            <span className="text-[10px] text-zinc-500">
              Auto-starts <span className="text-zinc-300">{formatLocalTime(engagement.openStartTime)}</span>
            </span>
          )}
          {isOpenFormat && engagement.openEndTime && engagement.status !== 'CLOSED' && engagement.status !== 'CANCELLED' && (
            <span className="text-[10px] text-zinc-500">
              Ends <span className="text-zinc-300">{formatLocalTime(engagement.openEndTime)}</span>
            </span>
          )}
          {isOpenFormat && engagement.graceSeconds != null && engagement.status === 'PENDING' && (
            <span className="text-[10px] text-zinc-500">
              Grace <span className="text-zinc-300">{engagement.graceSeconds}s</span>
            </span>
          )}
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
              ...
              <div>
                ...
                <p className="text-zinc-300 text-sm">
                  {engagement.auctionType}{isOpenFormat ? ' (Open)' : ''}
                </p>
              </div>
              <div>
              ...
              {!isOpenFormat && userRole !== 'BIDDER' && engagement.currentLiveRate != null && (
                <div>
              ...
              <div>
                <span className="text-zinc-500 block mb-1 text-xs">Auctioneer</span>
                <p className="text-zinc-300 text-sm">{auctioneerDisplay}</p>
              </div>
              {engagement.bearerEmail && (
                <div>
                  <span className="text-zinc-500 block mb-1 text-xs">Auctioneer Email</span>
                  <p className="text-zinc-300 text-sm break-all">{engagement.bearerEmail}</p>
                </div>
              )}
              ...
              {isOpenFormat && engagement.openStartTime && (
                <div>
                  <span className="text-zinc-500 block mb-1 text-xs">Auto-Start Time</span>
                  <p className="text-zinc-300 text-sm">{formatLocalTime(engagement.openStartTime)}</p>
                </div>
              )}
              {isOpenFormat && engagement.openEndTime && (
                <div>
                  <span className="text-zinc-500 block mb-1 text-xs">Scheduled End</span>
                  <p className="text-zinc-300 text-sm">{formatLocalTime(engagement.openEndTime)}</p>
                </div>
              )}
              {isOpenFormat && engagement.graceSeconds != null && (
                <div>
                  <span className="text-zinc-500 block mb-1 text-xs">Grace per Item</span>
                  <p className="text-zinc-300 text-sm">{engagement.graceSeconds}s</p>
                </div>
              )}
            </div>
          </div>
